...
// $update = query("SELECT * FROM (((((`account` LEFT JOIN `account_level` ON `account`.`level_id` = `account_level`.`level_id`) LEFT JOIN `house_data` ON `house_data`.`user_id` = `account`.`user_id`) LEFT JOIN `house_data_status` ON `house_data`.`status_id` = `house_data_status`.`status_id`) LEFT JOIN `house_photo` ON `house_photo`.`house_id` = `house_data`.`house_id`) LEFT JOIN `sertivicate` ON `sertivicate`.`house_id` = `house_data`.`house_id`) WHERE house_id = '$house_id'")[0];

if (isset($_POST["submit"])) {
    update($_POST, $house_id);
                ...
                <div class="modal fade" id="backModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog modal-dialog-centered modal-dialog-scrollable">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                <a class="btn btn-primary" href="details_seller.php?house_id=<?= $update['house_id']; ?>" style="background-color: #20C997;">Back</a>
                            </div>
                        </div>
                    </div>
                </div>
